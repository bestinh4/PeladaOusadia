...
const DrawScreen: React.FC<DrawScreenProps> = ({ players, onNavigate }) => {
  ...
  const [teams, setTeams] = useState<{ name: string; players: Player[]; average: number }[]>([]);
  ...
  const handleDraw = () => {
    ...
    const newTeams: { name: string; players: Player[]; average: number }[] = Array.from({ length: numTeams }, (_, i) => ({
    ...
    if (balanceLevels) {
      ...
      pool.forEach((player, index) => {
        const teamIndex = Math.floor(index / numTeams) % 2 === 0 
          ? index % numTeams 
    ...
    <div className="h-full bg-background overflow-y-auto no-scrollbar pb-32">
        ...
        <div className="bg-white border border-slate-100 rounded-3xl p-6 space-y-6 shadow-sm">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4 bg-slate-50 p-2 rounded-2xl border border-slate-100">
              <button onClick={() => setNumTeams(Math.max(2, numTeams - 1))} className="size-8 bg-white border border-slate-200 rounded-xl text-secondary font-bold hover:bg-slate-50 transition-colors">-</button>
              <span className="text-lg font-black text-secondary w-4 text-center">{numTeams}</span>
              <button onClick={() => setNumTeams(Math.min(4, numTeams + 1))} className="size-8 bg-secondary text-white rounded-xl font-bold hover:brightness-110 transition-all">+</button>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <button 
              onClick={() => setBalanceLevels(!balanceLevels)}
              className={`w-12 h-6 rounded-full relative transition-colors ${balanceLevels ? 'bg-primary' : 'bg-slate-200'}`}
            >
      ...
      <div className="fixed bottom-32 left-0 w-full px-6 z-40">
        <button 
          ...
          className="w-full h-15 btn-primary rounded-2xl flex items-center justify-center gap-3 disabled:opacity-50 disabled:grayscale"
        >
