      ...
      <div className="w-full max-w-sm bg-white/80 backdrop-blur-xl rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.1)] p-10 flex flex-col items-center gap-10 relative z-10 animate-fade-in-up border border-white/40">
        ...
        <div className="w-full space-y-4">
          ...
          <button 
            ...
            className={`w-full h-15 bg-white border border-gray-100 shadow-sm rounded-2xl flex items-center justify-center gap-3 hover:bg-gray-50 hover:shadow-md transition-all active:scale-95 group ${isAuthenticating ? 'opacity-70 cursor-wait' : ''}`}
          >
             {isAuthenticating ? (
             ...
             <span className="font-bold text-gray-700">{isAuthenticating ? 'Authenticating...' : 'Continue with Google'}</span>
          </button>
          ...
          <button 
            onClick={onDemoMode}
            className="w-full flex items-center justify-center gap-2 h-12 rounded-xl text-xs font-bold text-navy hover:text-primary transition-all active:scale-95 bg-gray-50/50"
          >
