  ...
  return (
    <div className="h-full bg-background flex flex-col relative">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar pb-40 px-6 pt-6">
        ...
        <div className="mb-6 animate-slide-up delay-100">
          <div className="relative">
            <input type="text" placeholder="Pesquisar atleta..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full h-14 bg-white border border-slate-200 rounded-2xl px-12 text-sm font-bold text-secondary placeholder:text-slate-300 focus:outline-none focus:border-primary transition-all shadow-sm" />
              ...
              <div key={player.id} className="flex items-center justify-between bg-white p-4 rounded-3xl border border-slate-100 shadow-sm list-item-hover">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-bold text-slate-300 uppercase tracking-widest">{isPaid ? 'Pagamento Confirmado' : isOverdue ? 'Atrasado 2 dias' : 'Pendente'}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <span className={`px-3 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest ${isPaid ? 'bg-success/10 text-success' : isOverdue ? 'bg-primary/10 text-primary' : 'bg-slate-50 text-slate-400'}`}>{isPaid ? 'Pago' : isOverdue ? 'Atrasado' : 'Aguardando'}</span>
      ...
      <div className="absolute bottom-10 left-0 w-full px-6 z-40 animate-slide-up pointer-events-none">
        <button className="w-full h-15 btn-primary rounded-2xl flex items-center justify-center gap-3 shadow-2xl pointer-events-auto">
