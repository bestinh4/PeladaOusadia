    ...
    <div className="h-full bg-background overflow-y-auto no-scrollbar pb-36 px-6">
      <header className="flex items-center justify-between pt-10 pb-6 sticky top-0 z-40 bg-background/80 backdrop-blur-xl -mx-6 px-6 border-b border-slate-100">
        <div className="flex items-center gap-3">
          <img src={CROATIA_LOGO} alt="Logo" className="size-10 object-contain" />
          <div>
            ...
            <p className="text-[9px] font-black text-primary uppercase tracking-[0.3em] mt-1">Elite Manager</p>
          </div>
        </div>
      ...
      <div className="space-y-8 mt-6">
            ...
            <div className="stadium-overlay rounded-[3rem] p-8 border border-white/10 relative overflow-hidden pro-shadow animate-scale-in">
              <div className="relative z-10">
                ...
                <div className="mb-10 text-center">
                  <h3 className="text-5xl font-black text-white italic tracking-tighter mb-1 leading-tight">{activeMatch.time}</h3>
        ...
        {activeMatch && (
          <div className="animate-slide-up delay-100">
            <button 
              ...
              className={`w-full h-16 rounded-3xl flex items-center justify-center gap-4 shadow-xl transition-all active:scale-95 disabled:opacity-50 ${isConfirmed ? 'bg-white border-2 border-slate-100 text-slate-400' : 'btn-elite text-white shadow-primary/20'}`}
            >
              <span className="material-symbols-outlined font-black text-[24px]">{isConfirmed ? 'cancel' : 'check_circle'}</span>
        ...
        <div className="grid grid-cols-2 gap-5 animate-slide-up delay-300 pb-12">
          <div onClick={() => onNavigate('players')} className="bg-white border border-slate-100 rounded-[2rem] p-6 space-y-4 pro-shadow group cursor-pointer active:scale-95 transition-all">
            ...
            <div>
              <p className="text-3xl font-black text-secondary italic leading-none">{activeMatch ? Math.max(activeMatch.limit - confirmedCount, 0) : '0'}</p>
          ...
          <div onClick={() => onNavigate('scout')} className="bg-white border border-slate-100 rounded-[2rem] p-6 space-y-4 pro-shadow group cursor-pointer active:scale-95 transition-all">
            ...
            <div>
              <p className="text-3xl font-black text-secondary italic leading-none">#{players.findIndex(p => p.id === currentPlayer?.id) + 1 || '?'}</p>
